    ...
    if (document.body.clientWidth > 860) {
        $(window).scroll(function () {
            ...
            if (false) {
                ...
                $('#bar').css('background', 'linear-gradient(to right, rgba(255, 0, 0, 0.75), rgba(255, 92, 208, 0.9), rgba(0, 217, 255, 0.90))')
            }
...
function bgAttr(b,a)
{
    var imgpath = '' + bg[Math.abs(bgindex % bg.length)] + '';
    a.find("img").attr("src",imgpath);
    a.attr("class","covers animated fadeIn slower");
    b.attr("class","covers animated fadeOut slower");
        // a.css("display","block");
        // setTimeout(function(){
        //     b.css("display","none");
        // },2500);
...
function nextBG() {
    var before = $(".covers").eq([Math.abs(bgindex % bg.length)]);
    bgindex = bgindex + 1
    var after = $(".covers").eq([Math.abs(bgindex % bg.length)]);
    bgAttr(before,after);
}
...
$(document).ready(function () {
    ...
    for(var i=0;i<bg.length;i++)
    {
        ...
        var imgpath = '' + bg[Math.abs(bgindex % bg.length)] + '';
        var div =$("<div></div>");
        div.attr("class","covers animated fadeOut slower");
        div.append($("<img>").attr("src", imgpath));
        bgindex = bgindex+1;
        centerbg.prepend(div);
    }
